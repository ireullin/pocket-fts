...
主要卡在寫入池的排隊深度與偶發 checkpoint，不是 fsync 頻率。

**結案，不繼續追。** 主要目標（降低 fsync 成本、讀取不被寫入卡住）已經達成，高併發 p99
當成已知限制記錄下來。之後若有人想繼續往下查，方向是：先驗證是不是真的是 checkpoint
造成的尾端延遲（例如量 `PRAGMA wal_checkpoint` 的回傳值、加上前後 log），不要在還沒驗證
假設之前就先調 `wal_autocheckpoint` 門檻；若確認是 checkpoint，再考慮調頁數門檻或評估寫入
池是否該從單一連線放寬（後者會重新引入 SQLite 鎖爭用風險，需要另外討論）。
