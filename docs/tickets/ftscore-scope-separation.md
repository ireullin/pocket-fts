...
**既有 collection 在 ftscore 端留下的死表。** 修復前建立的 collection，若 schema 剛好零
個 `indexed` 欄位，之前仍會在 ftscore 那邊建出一張對應的 FTS5 表。這次修完之後，寫入不再
同步到那張表，它會變成死資料（不再更新，但也不會被自動刪除，純粹佔用 `.indices` 檔案的
磁碟空間）。這次不處理，需要清理的話另開工作。
